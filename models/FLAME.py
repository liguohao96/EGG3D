...
class FLAME(nn.Module):
    ...
    def __init__(self, config):
        ...
        if "additional_tri" in config:
            print(f"using additional tri from {config['additional_tri']}")
            add_tri = np.loadtxt(config["additional_tri"], dtype=flame_model.f.dtype)

            flame_model.f = np.concatenate([flame_model.f, add_tri], axis=0)
    ...
    def forward(self, shape_params=None, expression_params=None, pose_params=None, neck_pose_params=None, eye_pose_params=None, with_joint_transform=False):
        """
            Input:
        ...
        vertices, _, joint_transform = lbs(betas, full_pose, template_vertices,
        ...
        if with_joint_transform is True:
            return vertices, landmarks2d, landmarks3d, joint_transform
        else:
...
class FLAMETex(nn.Module):
    ...
    def __init__(self, config):
        ...
        texture_mean  = texture_mean.reshape(texture_mean.size(0), 512, 512, 3)[:, :, :, [2,1,0]]
        texture_basis = texture_basis.reshape(texture_basis.size(0), 512, 512, 3, n_tex)[:, :, :, [2,1,0]]
        texture_mean  = texture_mean.permute(0, 3, 1, 2).flatten(1)        # 1, 3*512*512
        texture_basis = texture_basis.permute(0, 3, 1, 2, 4).flatten(1,-2) # 1, 3*512*512, n_tex
    ...
    def forward(self, texcode):
        ...
        texture = texture.reshape(texcode.shape[0], 3, 512, 512)
